...
class NotesService implements INotesService {
  ...
  private readonly _collaborationService: CollaborationsService
  ...
  async addNote ({ title, body, tags, owner }: NotePayload): Promise<string> {
    const id = nanoid(16)
    const createdAt = new Date().toISOString()
    const updatedAt = createdAt

    const query: QueryConfig<Array<string | string[]>> = {
    ...
    if (result.rows[0].id === '' || result.rows[0].id === undefined) {
  ...
  async getNotes (owner: string): Promise<MappedNote[]> {
    const query = {
  ...
  async getNoteById (id: string): Promise<MappedNote> {
    const query = {
  ...
  async editNoteById (id: string, { title, body, tags }: Omit<NotePayload, 'owner'>): Promise<void> {
  ...
  async verifyNoteOwner (id: string, owner: string): Promise<void> {
    const query = {
  ...
  async verifyNoteAccess (noteId: string, userId: string): Promise<void> {
    try {
